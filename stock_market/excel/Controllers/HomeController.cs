...
namespace ImportExcelFIle.DotNETCore.Controllers
{
    public class HomeController : Controller
    {
        ...
        public HomeController(ILogger<HomeController> logger, IHostingEnvironment hostingEnvironment, IImportExcelRepository repo)
        {
            _logger = logger;
                    ...
                    for (int i = (sheet.FirstRowNum + 1); i <= sheet.LastRowNum; i++) //Read Excel File
                    {
                        ...
                        for (int j = row.FirstCellNum; j < cellCount; j++)
                        {
                            if (row.GetCell(j) != null)
                            {
                                ...
                                l.Add(row.GetCell(j).ToString());
                            }
                        }
                        StockPrice sp = new StockPrice();
                        //sp.Id = (int)Int64.Parse(l[0]);
                        sp.Code = l[0];
                        sp.StockExchange = l[1];
                        sp.Price = l[2];
                        sp.Date = l[3];
                        sp.Time = l[4];
                        _repo.Add(sp);
        ...
        static async Task CallWebAPIAsync(int s)
        {
            using (var client = new HttpClient())
            {
                ...
                HttpResponseMessage response = await client.PostAsJsonAsync("", s);
